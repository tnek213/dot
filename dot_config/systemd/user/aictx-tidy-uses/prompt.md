Shared slug-tidy rules. Both the daily batch (`run.sh`) and the manual
`aictx-tidy-one` shell function include this file in the prompt they
send to Claude. Eligibility filtering, candidate selection, and output
format live in the caller — this file is rules only.
